...
void EndLess(){     // Function that makes random led light up
    ...
    */
	...
	while (digitalRead(Ran_Button) == HIGH){    // While the right button has not been pressed, it is turned on

        if ( Ran_Button == LED1  && digitalRead(K1) == HIGH){   // If the correct button is pressed, the the led is turned of and the function goes on
